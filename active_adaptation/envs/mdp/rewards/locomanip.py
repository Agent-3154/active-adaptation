...
class impedance_base_vel(Reward):
    ...
    def compute(self) -> torch.Tensor:
        diff = (self.command_manager.command_linvel_base_w[:, :2] - self.asset.data.root_link_lin_vel_w[:, :2])
...
class impedance_ee_pos_b(Reward):
    ...
    def update(self):
        pos_ee_w = self.asset.data.body_link_pos_w[:, self.command_manager.ee_body_id] - self.asset.data.root_pos_w
...
class impedance_ee_pos_w(Reward):
    ...
    def compute(self) -> torch.Tensor:
        ee_pos_w = self.asset.data.body_link_pos_w[:, self.command_manager.ee_body_id]
...
class impedance_ee_vel_b(Reward):
    ...
    def update(self):
        pos_ee_w = self.asset.data.body_link_pos_w[:, self.command_manager.ee_body_id] - self.asset.data.root_pos_w
        root_link_ang_vel_w_only_yaw = self.asset.data.root_link_ang_vel_w.clone()
        root_link_ang_vel_w_only_yaw[:, :2] = 0.0
        coriolis_vel_ee_w = self.asset.data.root_link_lin_vel_w + torch.cross(
            root_link_ang_vel_w_only_yaw, 
            pos_ee_w,
            dim=-1,
        )
...
#     def compute(self) -> torch.Tensor:
#         diff = (self.command_manager.command_yaw_vel - self.asset.data.root_link_ang_vel_w[:, 2:3])
#         r = torch.exp(- diff.abs() / self.l)
#         return r
...
class ee_forward(Reward):
    ...
    def compute(self) -> torch.Tensor:
        base_fwd = quat_rotate(yaw_quat(self.asset.data.root_link_quat_w), self.base_fwd_vec)[:, :2]
