...
@lru_cache(maxsize=1)
def _load_asset():
    # Registering H2 must not download its assets when another robot is used.
    from mjhub import resolve_asset_reference

    xml_path = resolve_asset_reference(H2_MJCF_REF)
    root = ET.parse(xml_path).getroot()
    body_names = list(dict.fromkeys(elem.attrib["name"] for elem in root.iter("body") if "name" in elem.attrib))
    joint_names = list(dict.fromkeys(elem.attrib["name"] for elem in root.iter("joint") if "name" in elem.attrib))
    return xml_path, body_names, joint_names
...
def make_mjlab_cfg():
    ...
    xml_path, body_names, joint_names = _load_asset()
