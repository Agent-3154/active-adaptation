...
def test_offscreen_renderer_receives_per_world_model_fields() -> None:
    ...
    with patch("mjlab.viewer.offscreen_renderer.OffscreenRenderer") as renderer_cls:
        renderer = MjlabSimAdapter(
            sim, viewer_cfg=cfg, scene=scene
        )._get_offscreen_renderer()
