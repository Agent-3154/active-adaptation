...
torchrun --nnodes=${WORLD_SIZE} --nproc_per_node=8 --node_rank=${RANK} --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} train_ppo.py \
  task=G1AilabRoomNavBlind task.num_envs=1024 \
  wandb.project=g1_room wandb.run_name=G1AilabRoomNavBlind \
  algo=ppo_blind headless=True total_frames=10_000_000_000
